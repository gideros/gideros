...
void ogl2SetupShaders()
{
    ...
    ShaderProgram::stdColor = new ogl2ShaderProgram(hdrVShaderCode,stdCVShaderCode,hdrFShaderCode,stdCFShaderCode,
                                      stdUniforms,stdAttributes);
...
ShaderTexture *ogl2ShaderEngine::createTexture(ShaderTexture::Format format,ShaderTexture::Packing packing,int width,int height,const void *data,ShaderTexture::Wrap wrap,ShaderTexture::Filtering filtering)
{
	return new ogl2ShaderTexture(format,packing,width,height,data,wrap,filtering);
}
...
#endif
			glBindFramebuffer(GL_FRAMEBUFFER, fbo?((ogl2ShaderBuffer *)fbo)->glid:0);
#ifdef OPENGL_DESKTOP
		else
			glBindFramebufferEXT(GL_FRAMEBUFFER, fbo?((ogl2ShaderBuffer *)fbo)->glid:0);
#endif
        currentBuffer=fbo;
        return previous;
...
void ogl2ShaderEngine::setColor(float r,float g,float b,float a)
{
