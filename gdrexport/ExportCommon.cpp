...
void ExportCommon::copyTemplate(QString templatePath,ExportContext *ctx)
{
    ...
    if (ctx->assetsOnly)
        Utilities::copyFolder(dir, ctx->outputDir, ctx->renameList, ctx->wildcards, ctx->replaceList, QStringList() << "libgideros.so" << "libgideros.a" << "gideros.jar" << "gideros.dll" << "libgideros.dylib" << "libgideros.1.dylib" << "gideros.WindowsPhone.lib" << "gideros.Windows.lib" << "WindowsDesktopTemplate.exe" << "MacOSXDesktopTemplate", QStringList());
    else
...
void ExportCommon::exportAssets(ExportContext *ctx,bool compileLua)
{
    ...
    for (std::size_t i = 0; i < ctx->fileQueue.size(); ++i)
    {
        ...
        if (QFileInfo(src).suffix().toLower() == "lua")
        {
            ...
            if (std::find(ctx->topologicalSort.begin(), ctx->topologicalSort.end(), std::make_pair(s2, true)) == ctx->topologicalSort.end())
            {
    ...
    if (true)
    {
        for (int i = 0; i < ctx->allfiles_abs.size(); ++i)
        {
            ...
            QByteArray encryptionKey = (ext == "lua") ? ctx->codeKey : ctx->assetsKey;
...
{
    QFile file(QDir::cleanPath(ctx->outputDir.absoluteFilePath("allfiles.txt")));
