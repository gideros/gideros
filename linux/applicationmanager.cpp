...
void NetworkManager::createFolder(const std::vector<char>& data)
{
	...
	_mkdirp(g_pathForFile(folderName.c_str()));
}
...
ApplicationManager::ApplicationManager() {
	...
	gpath_addDrivePrefix(2, "|t|");
	    ...
	    std::string docsPath=std::string(getenv("HOME"))+"/.gideros";
	    docsPath=docsPath+"/"+PATH_AppName;
		_mkdirp(docsPath.c_str());
		docsPath=docsPath+"/";
	    gpath_setDrivePath(1,docsPath.c_str());
		externalDir_=PATH_Cache;
...
#include "netendian.h"
	void ApplicationManager::play(const char *gapp) {
		...
		_mkdirp(documents.c_str());
		_mkdirp(temporary.c_str());
