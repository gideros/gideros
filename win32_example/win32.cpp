...
void W32Screen::setSize(int w, int h) {
	...
	SetWindowPos(wnd, HWND_TOP, 0, 0, rect.right - rect.left,
...
void W32Screen::setPosition(int x, int y) {
	...
	AdjustWindowRect(&rect, WS_OVERLAPPEDWINDOW, FALSE);
	SetWindowPos(wnd, HWND_TOP, rect.left, rect.top, rect.right - rect.left,
...
W32Screen::W32Screen(Application *application, HINSTANCE hInstance) :
		Screen(application) {
	if (!W32Class) {
		WNDCLASSEX wndclass;
...
Screen* W32ScreenManager::openScreen(Application *application, int id) {
	return (Screen*) SendMessage(hwndcopy, WM_USER + 0x10, 0,
			(LPARAM) application);
}

// ######################################################################
static bool ExtensionsLoaded = false;
static bool LoadOpenGLExtensions(void) {
	if (ExtensionsLoaded)
		return true;
	...
	WNDCLASSA window_class = { .style = CS_HREDRAW | CS_VREDRAW | CS_OWNDC,
			.lpfnWndProc = DefWindowProcA, .hInstance = GetModuleHandle(0),
...
void EnableOpenGL(HWND hWnd, HDC *hDC, HGLRC *hRC) {
	...
	*hDC = GetDC(hWnd);
	...
	int gl33_attribs[] =
			...
			WGL_CONTEXT_PROFILE_MASK_ARB,
					WGL_CONTEXT_COMPATIBILITY_PROFILE_BIT_ARB, 0, };
	...
	*hRC = wglCreateContextAttribsARB(*hDC, 0, gl33_attribs);
	...
	if (strstr(_wglGetExtensionsStringEXT(), "WGL_EXT_swap_control_tear")
			== NULL)
		vsyncVal = 1;
	else
		vsyncVal = -1;
...
void render() {
	wglMakeCurrent(hDC, hRC);
	if (glChanged) {
		s_applicationManager->surfaceChanged(glWidth, glHeight,
				glWidth > glHeight ? 90 : 0);
		glChanged = false;
	}
	...
	/*  else if (iMsg==WM_GETMINMAXINFO) {
	 ...
	 return 0;
		...
		if (wParam & MK_CONTROL)
			m |= GINPUT_CTRL_MODIFIER;
		if (wParam & MK_SHIFT)
			m |= GINPUT_SHIFT_MODIFIER;
		if (GetKeyState(VK_MENU) & 0x8000)
			m |= GINPUT_ALT_MODIFIER;
		...
		return 0;
	} else if ((iMsg == WM_POINTERUPDATE) || (iMsg == WM_POINTERENTER)
			|| (iMsg == WM_POINTERLEAVE) || (iMsg == WM_POINTERUP)
			|| (iMsg == WM_POINTERDOWN) || (iMsg == WM_POINTERWHEEL)
		...
		bool contact =
				(iMsg == WM_POINTERCAPTURECHANGED) ?
						false : IS_POINTER_INCONTACT_WPARAM(wParam);
		float pressure = 0;
		int touchType = 2;
		...
		if (GetPointerType(tid, &ptype)) {
			...
			if (pi) {
				if (pi->dwKeyStates & POINTER_MOD_SHIFT)
					m |= 1;
				if (pi->dwKeyStates & POINTER_MOD_CTRL)
					m |= 4;
			}
		}
		...
		if (iMsg == WM_POINTERCAPTURECHANGED) {
			if (pcontact)
				ginputp_touchesCancel(pt.x, pt.y, tid, pressure, touchType,
						touch_xs.size(), touch_xs.data(), touch_ys.data(),
						touch_ids.data(), touch_pressures.data(),
						touch_types.data(), m, b);
		} else if (iMsg == WM_POINTERWHEEL)
			ginputp_mouseWheel(pt.x, pt.y, b, GET_WHEEL_DELTA_WPARAM(wParam),
					m);
		else if (iMsg == WM_POINTERUP)
			ginputp_touchesEnd(pt.x, pt.y, tid, pressure, touchType,
					touch_xs.size(), touch_xs.data(), touch_ys.data(),
					touch_ids.data(), touch_pressures.data(),
					touch_types.data(), m, b);
		else if (iMsg == WM_POINTERDOWN)
			ginputp_touchesBegin(pt.x, pt.y, tid, pressure, touchType,
					touch_xs.size(), touch_xs.data(), touch_ys.data(),
					touch_ids.data(), touch_pressures.data(),
					touch_types.data(), m, b);
			...
			if (contact)
				ginputp_touchesMove(pt.x, pt.y, tid, pressure, touchType,
						touch_xs.size(), touch_xs.data(), touch_ys.data(),
						touch_ids.data(), touch_pressures.data(),
						touch_types.data(), m, b);
			else
