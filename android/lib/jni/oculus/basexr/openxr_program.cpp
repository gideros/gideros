...
struct OpenXrProgram : IOpenXrProgram {
    ...
    void InitializeActions() {
        ...
        // Create actions.
        {
            ...
            //Index Action
            strcpy_s(actionInfo.actionName, "point_object");
            strcpy_s(actionInfo.localizedActionName, "Point Object");
            actionInfo.countSubactionPaths = uint32_t(m_input.handSubactionPath.size());
            actionInfo.subactionPaths = m_input.handSubactionPath.data();
            CHECK_XRCMD(xrCreateAction(m_input.actionSet, &actionInfo, &m_input.indexAction));

            actionInfo.actionType = XR_ACTION_TYPE_VECTOR2F_INPUT;
            strcpy_s(actionInfo.actionName, "thumbstick");
            strcpy_s(actionInfo.localizedActionName, "Thumbstick movement");
            actionInfo.countSubactionPaths = uint32_t(m_input.handSubactionPath.size());
            actionInfo.subactionPaths = m_input.handSubactionPath.data();
            CHECK_XRCMD(xrCreateAction(m_input.actionSet, &actionInfo, &m_input.stickAction));
            ...
            actionInfo.countSubactionPaths = uint32_t(m_input.handSubactionPath.size());
            actionInfo.subactionPaths = m_input.handSubactionPath.data();
            CHECK_XRCMD(xrCreateAction(m_input.actionSet, &actionInfo, &m_input.btnAAction));
            strcpy_s(actionInfo.actionName, "b_button");
            strcpy_s(actionInfo.localizedActionName, "B button");
            actionInfo.countSubactionPaths = uint32_t(m_input.handSubactionPath.size());
            actionInfo.subactionPaths = m_input.handSubactionPath.data();
            CHECK_XRCMD(xrCreateAction(m_input.actionSet, &actionInfo, &m_input.btnBAction));
            strcpy_s(actionInfo.actionName, "x_button");
            strcpy_s(actionInfo.localizedActionName, "X button");
            actionInfo.countSubactionPaths = uint32_t(m_input.handSubactionPath.size());
            actionInfo.subactionPaths = m_input.handSubactionPath.data();
            CHECK_XRCMD(xrCreateAction(m_input.actionSet, &actionInfo, &m_input.btnXAction));
            strcpy_s(actionInfo.actionName, "y_button");
            strcpy_s(actionInfo.localizedActionName, "Y button");
            actionInfo.countSubactionPaths = uint32_t(m_input.handSubactionPath.size());
            actionInfo.subactionPaths = m_input.handSubactionPath.data();
            CHECK_XRCMD(xrCreateAction(m_input.actionSet, &actionInfo, &m_input.btnYAction));
        ...
        std::array<XrPath, Side::COUNT> stickValuePath;
...
//        CHECK_XRCMD(xrStringToPath(m_instance, "/user/hand/right/input/grip/pose", &posePath[Side::RIGHT]));
        CHECK_XRCMD(xrStringToPath(m_instance, "/user/hand/left/input/aim/pose", &posePath[Side::LEFT]));
        CHECK_XRCMD(xrStringToPath(m_instance, "/user/hand/right/input/aim/pose", &posePath[Side::RIGHT]));
        ...
        CHECK_XRCMD(xrStringToPath(m_instance, "/user/hand/left/input/thumbstick", &stickValuePath[Side::LEFT]));
        CHECK_XRCMD(xrStringToPath(m_instance, "/user/hand/right/input/thumbstick", &stickValuePath[Side::RIGHT]));
        ...
        // Suggest bindings for the Oculus Touch.
        {
            ...
            std::vector<XrActionSuggestedBinding> bindings{{{m_input.grabAction, squeezeValuePath[Side::LEFT]},
															...
															{m_input.indexAction, triggerValuePath[Side::LEFT]},
                                                            ...
                                                            {m_input.btnYAction, yClickPath[Side::LEFT]},
                                                            {m_input.stickAction, stickValuePath[Side::LEFT]},
                                                            {m_input.stickAction, stickValuePath[Side::RIGHT]},
                                                            {m_input.vibrateAction, hapticPath[Side::LEFT]},
                                                            {m_input.vibrateAction, hapticPath[Side::RIGHT]}}};
            XrInteractionProfileSuggestedBinding suggestedBindings{XR_TYPE_INTERACTION_PROFILE_SUGGESTED_BINDING};
    ...
    void PollActions() override {
        ...
        for (auto hand : {Side::LEFT, Side::RIGHT}) {
            ...
            XrActionStateVector2f vecValue{XR_TYPE_ACTION_STATE_VECTOR2F};
            CHECK_XRCMD(xrGetActionStateFloat(m_session, &getInfo, &grabValue));
            if (grabValue.isActive == XR_TRUE) {
                m_input.handGrip[hand] = grabValue.currentState;
                if (grabValue.currentState > 0.5f) bmap|=0x04000000;
/*                if (grabValue.currentState > 0.9f) {
            ...
            getInfo.action = m_input.indexAction;
            CHECK_XRCMD(xrGetActionStateFloat(m_session, &getInfo, &grabValue));
            if (grabValue.isActive == XR_TRUE) {
                if (grabValue.currentState > 0.5f) bmap|=0x20000000;
                m_input.handIndex[hand] = grabValue.currentState;
            }

            getInfo.action = m_input.stickAction;
            CHECK_XRCMD(xrGetActionStateVector2f(m_session, &getInfo, &vecValue));
            if (vecValue.isActive == XR_TRUE) {
                m_input.handStick[hand] = vecValue.currentState;
            }

            getInfo.action = m_input.btnAAction;
            CHECK_XRCMD(xrGetActionStateBoolean(m_session, &getInfo, &bValue));
            if (bValue.currentState == XR_TRUE) bmap|=1;
            getInfo.action = m_input.btnBAction;
            CHECK_XRCMD(xrGetActionStateBoolean(m_session, &getInfo, &bValue));
            if (bValue.currentState == XR_TRUE) bmap|=2;
            getInfo.action = m_input.btnXAction;
            CHECK_XRCMD(xrGetActionStateBoolean(m_session, &getInfo, &bValue));
            if (bValue.currentState == XR_TRUE) bmap|=0x100;
            getInfo.action = m_input.btnYAction;
            CHECK_XRCMD(xrGetActionStateBoolean(m_session, &getInfo, &bValue));
            if (bValue.currentState == XR_TRUE) bmap|=0x200;
            m_input.handButtons[hand] = bmap;
