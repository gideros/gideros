...
PluginsChooser::PluginsChooser(QSet<ProjectProperties::Plugin> selection,
		QWidget *parent) :
		QDialog(parent), ui(new Ui::PluginsChooserDialog) {
	sel = selection;

	QSet < QString > enabledPlugins;
	for (QSet<ProjectProperties::Plugin>::iterator it = sel.begin();
			it != sel.end(); it++) {
		if ((*it).enabled)
	...
	connect(ui->plugins, SIGNAL(itemSelectionChanged()), this,
...
void PluginsChooser::onShowUserPlugins()
{
	QDir shared(
				QStandardPaths::writableLocation(
						QStandardPaths::GenericDataLocation));
...
void PluginsChooser::onShowStandardPlugins()
{
	QDir sourceDir(ALL_PLUGINS_PATH);
