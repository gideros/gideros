	...
	virtual void packTextures(int* pwidth, int* pheight, int border, bool forceSquare)
	{
		...
		for (int i = 0; i < max * 2 + 1; ++i)
		{
			...
			for (int j = 0; j < jmax; ++j)
			{
                ...
                res.push_back(std::make_pair(width, height));
            }
        }

        std::stable_partition(res.begin(), res.end(), pred);

        for (size_t i = 0; i < res.size(); ++i)
        {
            int width = res[i].first;
            int height = res[i].second;
...
private:
    ...
    static inline bool pred(const std::pair<int, int>& p)
    {
        return p.first <= 1024 && p.second <= 1024;
    }
