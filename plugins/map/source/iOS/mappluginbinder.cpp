...
So why do we map a Lua function to a static function that calls a class method, which calls a static function which calls a class method,
which calls a native Java method? Couldn't one static function call one class method which calls the native Java?

This is how the IAB plugin does it, and that's been cited as a good example to base other plugins on.  Maybe there's an inherent reason it
must be this way, forced by either the Gideros plugin system itself.  Perhaps this was only necessary for IAB since it defined an interface
that could be implemented by frameworks for different app stores.  I'll spend no more time trying to optimize this.  It mirrows the levels in
IAB, and it works.  People more familiar with Gideros plugins are welcome to improve it.
...
{
public:
    MAPPLUGINBINDER(lua_State *L, const char *mapplugin) : L(L)
    {
		mapplugin_ = strdup(mapplugin);
		gmapplugin_initialize(mapplugin);
    }
...
static int init(lua_State* L)
{
	...
	MAPPLUGINBINDER *mapplugin = new MAPPLUGINBINDER(L, iabp);
