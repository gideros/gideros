...
G_API void game_onReportScoreError(const char *caller, const char *value, const char *error, long score);
G_API void game_onLoadAchievementsComplete(const char* call, Achievement* ach);
G_API void game_onLoadAchievementsError(const char *caller, const char *value);
G_API void game_onLoadScoresComplete(const char* call, const char* jid, const char* jname, Score* score);
G_API void game_onLoadScoresError(const char *caller, const char *value, const char *error);
G_API void game_onStateLoaded(const char* type, int key, const void* data, size_t dataSize, int fresh);
G_API void game_onStateError(const char *type, int key, const char *error);
G_API void game_onStateConflict(const char *type, int key, const char *ver, const void* localState, size_t localSize, const void* serverState, size_t serverSize);
G_API void game_onStateDeleted(const char *type, int key);
