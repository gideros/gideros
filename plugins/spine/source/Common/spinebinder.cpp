...
void _spAtlasPage_createTexture (spAtlasPage* self, const char* path) {
	...
	TextureData *tex=application->getTextureManager()->createTextureFromFile(path, parameters, false);
...
void _spAtlasPage_disposeTexture (spAtlasPage* self) {
	application->getTextureManager()->destroyTexture((TextureData *)self->rendererObject);
}
...
struct Color4u {
	...
	unsigned char b;
	unsigned char a;
	Color4u(unsigned char r1,unsigned char g1,unsigned char b1,unsigned char a1): r(r1),g(g1),b(b1),a(a1) { };
...
{
public:
    SpineSprite(LuaApplication* application,const char *fileName, const char *atlasName,float scale);
	virtual ~SpineSprite();
	SpriteProxy *proxy_;
...
private:
    ...
    enum BlendMode {NORMAL,ADDITIVE,MULTIPLY,SCREEN} cblend;
};
...
SpineSprite::SpineSprite(LuaApplication* application,const char *fileName, const char *atlasName,float scale) : vertexEffect(0),
		worldVertices(0), clipper(0)
{
	...
	worldVertices = (float *)malloc(SPINE_MESH_VERTEX_COUNT_MAX*sizeof(float));
...
void SpineSprite::drawMesh() {
	...
	if (p)
	{
		p->setData(ShaderProgram::DataVertex, ShaderProgram::DFLOAT, 2,&gvertices[0], gvertices.size(), true, NULL);
        p->setData(ShaderProgram::DataColor, ShaderProgram::DUBYTE, 4,&gcolors[0],gcolors.size(), true, NULL);
        p->setData(ShaderProgram::DataTexture, ShaderProgram::DFLOAT, 2,&gtexcoords[0],gtexcoords.size(), true, NULL);

		float textureInfo[4]={0,0,0,0};
		if (ctexture)
		{
			gtexture_get_engine()->bindTexture(0,gtexture_getInternalTexture(ctexture->gid));
			textureInfo[0]=(float)ctexture->width / (float)ctexture->exwidth;
			textureInfo[1]=(float)ctexture->height / (float)ctexture->exheight;
...
void SpineSprite::doDraw(const CurrentTransform& , float _UNUSED(sx), float _UNUSED(sy), float _UNUSED(ex), float _UNUSED(ey))
{
	...
	for (int i = 0; i < skeleton->slotsCount; ++i) {
		...
		if (attachment->type == SP_ATTACHMENT_REGION) {
			...
			indices = quadIndices;
			indicesCount = 6;
			texture = (TextureData *)((spAtlasRegion*)regionAttachment->rendererObject)->page->rendererObject;
			attachmentColor = &regionAttachment->color;

		} else if (attachment->type == SP_ATTACHMENT_MESH) {
			spMeshAttachment* mesh = (spMeshAttachment*)attachment;
			if (mesh->super.worldVerticesLength > SPINE_MESH_VERTEX_COUNT_MAX) continue;
			texture = (TextureData*)((spAtlasRegion*)mesh->rendererObject)->page->rendererObject;
		...
		if (ctexture == 0) ctexture = texture;
...
int setAnimation(lua_State *L) {
    Binder binder(L);
    SpriteProxy* psp = static_cast<SpriteProxy*>(binder.getInstance("SpineSprite",1));
    SpineSprite* ps = static_cast<SpineSprite*>(psp->getContext());
    if (lua_isnoneornil(L,3))
        spAnimationState_setEmptyAnimation(ps->state, luaL_checkinteger(L,2), luaL_optnumber(L,4,0));
    else
    {
        spTrackEntry *te=spAnimationState_setAnimationByName(ps->state, luaL_checkinteger(L,2), luaL_checkstring(L,3),lua_toboolean(L,5));
...
int addAnimation(lua_State *L) {
    ...
    SpineSprite* ps = static_cast<SpineSprite*>(psp->getContext());
    if (lua_isnoneornil(L,3))
        spAnimationState_addEmptyAnimation(ps->state, luaL_checkinteger(L,2), luaL_optnumber(L,4,0),luaL_optnumber(L,6,0.0));
    else
    {
        spTrackEntry *te=spAnimationState_addAnimationByName(ps->state, luaL_checkinteger(L,2), luaL_checkstring(L,3),lua_toboolean(L,5),luaL_optnumber(L,6,0.0));
        if ((!lua_isnoneornil(L,4))&&te)
            te->mixDuration=luaL_checknumber(L,4);
    }
...
int setMix(lua_State *L) {
    ...
    if (lua_isnone(L,3))
        ...
        spAnimationStateData_setMixByName(ps->state->data,luaL_checkstring(L,2),luaL_checkstring(L,3),luaL_checknumber(L,4));
    return 0;
}
...
binder.createClass("SpineSprite", "Sprite", createSpineSprite, destroySpineSprite, SpineSprite_functionList);
